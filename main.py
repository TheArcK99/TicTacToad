import os
import random
from network import TicTacToeModel
import copy
...
class Game:
    ...
    def simulateManyGames(self, playerToMove, numberOfGames):
        ...
        with open('play.txt', 'a') as f:
            f.write('X Wins: ' + str(int(playerXWins * 100/totalWins)) + '%')
            f.write('\n')
            f.write('O Wins: ' + str(int(playerOWins * 100 / totalWins)) + '%')
            f.write('\n')
            f.write('Draws: ' + str(int(draws * 100 / totalWins)) + '%')
            f.write('\n')
    ...
    def simulateManyNeuralNetworkGames(self, nnPlayer, numberOfGames, model):
        ...
        print('Draws: ' + str(int(draws * 100 / totalWins)) + '%')
        with open('play.txt', 'a') as f:
            f.write('X Wins: ' + str(int(nnPlayerWins * 100/totalWins)) + '%')
            f.write('\n')
            f.write('O Wins: ' + str(int(randomPlayerWins * 100 / totalWins)) + '%')
            f.write('\n')
            f.write('Draws: ' + str(int(draws * 100 / totalWins)) + '%')
            f.write('\n')
